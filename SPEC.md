...
8. **M8 — Server deploy.** Live on a GPU host: `uv sync --extra marker`, `[network] host="0.0.0.0"` + `allowed_hosts` for mesh (Tailscale/WireGuard) reach, systemd `--user` unit. Not a code change; deploy-only.
...
10. **M10 — Marker thermal safety.** `marker_batch_size` caps surya's per-phase batch sizes (via `RECOGNITION_BATCH_SIZE` etc. env vars, set before marker import) so a small, thermal-constrained GPU stays under thermal budget. `marker_cooldown_seconds` sleeps between marker runs to help folder ingest recover between docs. Composes with an OS-level `nvidia-smi -pl` power cap applied via a systemd unit.
